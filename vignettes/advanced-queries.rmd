...
2. **At read time** — filter rows with `where` in `read_dewey_duck()`, or bypass it entirely and write raw DuckDB SQL
...
The cheapest query is one that never downloads data you don't need. Use `where` and `select` in `download_dewey_duck()` to trim the dataset before it hits your disk:
...
## Simple Reads with `read_dewey_duck()`

For basic filtering on already-downloaded data, use the `where` argument in `read_dewey_duck()`:

```{r, eval=FALSE}
path <- "C:/your/path/to/dewey-downloads/airline-employment"

# Filter to Major carriers only
df <- read_dewey_duck(path, where = "CARRIER_GROUP = 'Major'")
```
...
Summarize average full-time and part-time employees per carrier:
...
Although this example is more complex than most users will need, SQL is much quicker than R's tidyverse for massive datasets. Should you have a predictable transformation, it may be wise to do it while you read in using duckdb.
